        ...
        std::make_shared<std::vector<visualization_msgs::msg::Marker>>()}
{
    ...
    // Push start and end as points into arrow
    shape.points.push_back(start_);
    shape.points.push_back(end_);

    // Set scale
    shape.scale.x = 0.0; // Remove shaft of arrow
    shape.scale.y = 1.0; // Head diameter
    shape.scale.z = 1.0; // Head length
...
auto Cone::resize_imple(AllAxis const new_size) -> void
{
    size_ = new_size;
...
auto Cone::set_colour_imple(Colour const new_colour) -> void
{
    switch (new_colour)
    {
        case Colour::red:
        ...
        case Colour::black:
            ...
            shapes_list_ptr_->at(0).color.g = 0.0;
            shapes_list_ptr_->at(0).color.b = 0.0;
            shapes_list_ptr_->at(0).color.a = 0.0;
            break;
...
auto Cone::get_colour_imple(void) const -> Colour
{
...
auto Cone::get_location_imple(void) const -> std::tuple<XAxis, YAxis, ZAxis>
{
    return std::tuple{
...
auto Cone::move_to_imple(XAxis const x) -> void
{
...
auto Cone::move_to_imple(YAxis const y) -> void
{
...
auto Cone::move_to_imple(ZAxis const z) -> void
{
...
auto Cone::get_display_markers_imple(void)
...
auto Cone::get_orientation_imple(void) const -> ZAxis
{
